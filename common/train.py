...
my_sampler = BalancedMultiSourceRandomSampler(
    whole_source, P.batch_p, P.local_rank, P.n_gpus
)
print(
    f"{P.local_rank} sampler_size: {len(my_sampler)}. Dataset_size: {len(whole_source)}"
)
train_loader = DataLoader(
    whole_source, sampler=my_sampler, batch_size=single_GPU_batch_size, **kwargs
)
...
(
    ...
    target_ds,
    _,
) = get_datasets_for_test(P)

source_test_sampler = DistributedSampler(
    source_ds, num_replicas=P.n_gpus, rank=P.local_rank
)
target_test_sampler = DistrilbutedSampler(
    target_ds, num_replicas=P.n_gpus, rank=P.local_rank
)
source_test_loader = DataLoader(
    source_ds, sampler=source_test_sampler, shuffle=False, batch_size=1, num_workers=4
)
target_test_loader = DataLoader(
    target_ds, sampler=target_test_sampler, shuffle=False, batch_size=1, num_workers=4
)
...
if P.optimizer == "sgd":
    optimizer = optim.SGD(
        model.parameters(), lr=P.lr_init, momentum=0.9, weight_decay=P.weight_decay
    )
    lr_decay_gamma = 0.1
...
if P.lr_scheduler == "cosine":  # default choice
    ...
    scheduler = lr_scheduler.MultiStepLR(
        optimizer, gamma=lr_decay_gamma, milestones=milestones
    )
else:
    raise NotImplementedError()
...
scheduler_warmup = GradualWarmupScheduler(
    optimizer, multiplier=1, total_epoch=P.warmup, after_scheduler=scheduler
)
...
if "st" in P.mode:
    assert (
        P.adain_ckpt is not None
    ), "You need to pass adain ckpt path using --adain_ckpt"

    # first we load style transfer model
    from models.adain import AdaIN

    adain_model = AdaIN()
    adain_model.load_state_dict(
        torch.load(P.adain_ckpt, map_location=lambda storage, loc: storage)
    )
    ...
    if P.multi_gpu:
        style_sampler = DistributedSampler(
            style_dataset, num_replicas=P.n_gpus, rank=P.local_rank
        )
        style_loader = DataLoader(
            style_dataset, sampler=style_sampler, batch_size=P.batch_size, **kwargs
        )
    else:
        style_loader = DataLoader(
            style_dataset, shuffle=True, batch_size=P.batch_size, **kwargs
        )
    ...
    P.simclr_aug_st = C.get_simclr_augmentation_crop_only(
