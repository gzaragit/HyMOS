...
def apply_simclr_aug(P, simclr_aug, videos, style_images):
    cpu_device = torch.device("cpu")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    batch_size = len(videos)
    adain_mask = torch.zeros((len(videos)), dtype=torch.bool)

    selected_content_videos = []
    selected_style_videos = []
    cnt_ids = []
    for i in range(len(videos)):
        adain_mask[i] = random.random() < P.adain_probability
        if adain_mask[i]:
            selected_content_videos.append(videos[i])
            selected_style_videos.append(
                style_images[int(random.random() * len(style_images))]
            )
            cnt_ids.append(i)
    ...
    if len(selected_content_images) > 0:
        selected_content_images = normalize_images(
            P, torch.stack(selected_content_images).to(device)
        )
        selected_style_images = normalize_images(
            P, torch.stack(selected_style_images).to(device)
        )
        with torch.no_grad():
            output_images = adain_model.generate(
                selected_content_images, selected_style_images, P.adain_alpha
            )

        images[adain_mask] = P.simclr_aug_st(
            denormalize_images(P, interpolate(output_images, images[0].shape[1:][::-1]))
        )
        del output_images
