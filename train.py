...
for its in range(start_its, P.iterations + 1):
    ...
    # train one iteration
    sim_loss, simclr_norm, train_loader, data_iter = train(
        P,
        its,
        model,
        criterion,
    ...
    if its % 10 == 0:
        ...
        logger.log(
            "[Iteration %3d] [Loss_Sim %5.2f] [SimclrNorm %f] [LR %f] [Avg time %.2fs] [ETA %dh%dm%ds]"
            % (
                its,
                losses["sim"].average,
                losses["norm"].average,
    ...
    if P.iterative and its in P.its_breakpoints:
        logger.log(
            f"Reached breakpoint iteration {its}. Start computing pseudo labels for new known samples"
        )

        known_mask, known_pseudo_labels, known_gt_labels = compute_confident_known_mask(
            P, model, source_test_loader, target_test_loader, logger=None
        )

        acc = accuracy_score(
            known_gt_labels[known_mask], known_pseudo_labels[known_mask]
        )
        logger.log(
            "Selected {} target samples as known. Classification accuracy for selected samples: {:.4f}".format(
                len(known_mask.nonzero()[0]), acc
            )
        )
        del known_gt_labels

        if len(known_mask.nonzero()[0]) > 0:
            train_sets, n_classes = get_dataset_2(
                P,
                train=True,
                target_known_mask=known_mask,
                target_known_pseudo_labels=known_pseudo_labels,
            )
